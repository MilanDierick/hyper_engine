...
#include "hyper/renderer/uniform_buffer.h"
...
namespace hp
{
	...
	void renderer_2d::init()
	{
		...
		s_data.quad_vertex_buffer = vertex_buffer::create(renderer_2d_data::max_vertices * sizeof(quad_vertex));
		...
		s_data.quad_vertex_buffer_base = new quad_vertex[renderer_2d_data::max_vertices];

		auto* quad_indices = new uint32_t[renderer_2d_data::max_indices];

		uint32_t offset = 0;
		for (uint32_t i = 0; i < renderer_2d_data::max_indices; i += 6)
		{
		...
		std::shared_ptr<index_buffer> quad_ib = index_buffer::create(quad_indices, renderer_2d_data::max_indices);
		...
		s_data.circle_vertex_buffer = vertex_buffer::create(renderer_2d_data::max_vertices * sizeof(circle_vertex));
		...
		s_data.circle_vertex_buffer_base = new circle_vertex[renderer_2d_data::max_vertices];

		// Lines
		s_data.line_vertex_array = vertex_array::create();

		s_data.line_vertex_buffer = vertex_buffer::create(renderer_2d_data::max_vertices * sizeof(line_vertex));
		s_data.line_vertex_buffer->set_layout(
		        {{shader_data_type::Float3, "a_Position"},
		         {shader_data_type::Float4, "a_Color"},
		         {shader_data_type::Int, "a_EntityID"}});
		s_data.line_vertex_array->add_vertex_buffer(s_data.line_vertex_buffer);
		s_data.line_vertex_buffer_base = new line_vertex[renderer_2d_data::max_vertices];

		s_data.white_texture        = texture_2d::create(1, 1);
		uint32_t white_texture_data = 0xffffffff;
		s_data.white_texture->set_data(&white_texture_data, sizeof(uint32_t));

		int32_t samplers[renderer_2d_data::max_texture_slots];
		for (uint32_t i = 0; i < renderer_2d_data::max_texture_slots; i++)
		{
	...
	void renderer_2d::flush()
	{
		...
		{
			auto data_size = (uint32_t) ((uint8_t*) s_data.quad_vertex_buffer_ptr - (uint8_t*) s_data.quad_vertex_buffer_base);
		...
		{
			auto data_size = (uint32_t) ((uint8_t*) s_data.circle_vertex_buffer_ptr - (uint8_t*) s_data.circle_vertex_buffer_base);
		...
		{
			auto data_size = (uint32_t) ((uint8_t*) s_data.line_vertex_buffer_ptr - (uint8_t*) s_data.line_vertex_buffer_base);

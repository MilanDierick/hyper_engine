...
namespace hp
{
		                                                                                                    ...
		                                                                                                    rotation)
	{
		...
		application::instance()->get_window().window_resized_event
	...
	void orthographic_camera_controller::on_update(uint64_t ticks)
	{
		...
		{
			m_camera_position.x -=
				cos(glm::radians(m_camera_rotation)) * m_camera_translation_speed * static_cast<float>(ticks);
			m_camera_position.y -=
				sin(glm::radians(m_camera_rotation)) * m_camera_translation_speed * static_cast<float>(ticks);
		}

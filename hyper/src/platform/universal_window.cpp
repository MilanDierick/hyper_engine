...
#include <events/mouse_event_args.h>
...
namespace hp
{
	...
	void universal_window::init(const window_parameters& parameters)
	{
		...
		m_context = std::make_unique<opengl_graphics_context>(m_window);
		m_context->init();
		...
		glfwSetScrollCallback(m_window, [](GLFWwindow*, const double x_offset, const double y_offset)
		{
		...
		glfwSetCursorPosCallback(m_window, [](GLFWwindow*, const double x_pos, const double y_pos)
		{
